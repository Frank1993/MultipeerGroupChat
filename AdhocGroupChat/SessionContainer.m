...
- (id)initWithDisplayName:(NSString *)displayName serviceType:(NSString *)serviceType
{
    if (self = [super init]) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(sendSessionInfomation) name:@"askForSessionInfomation" object:nil];
    }
    return self;
}
-(void)sendSessionInfomation
{
    NSMutableArray * peerIdArray = [[NSMutableArray alloc] init];
    for (MCPeerID * peerId in self.session.connectedPeers) {
        [peerIdArray addObject:peerId.displayName];
    }
    [[NSNotificationCenter defaultCenter] postNotificationName:@"SessionInfomaiton" object:peerIdArray];
}
...
- (Transcript *)sendMessage:(NSString *)message toPeerId:(NSString *)peerIdDisplayname
{
    ...
    NSError *error;
    NSArray * peerIds;
    if (peerIdDisplayname ==nil) {
        peerIds =[NSArray arrayWithArray:self.session.connectedPeers];
    }else
    {
        for (MCPeerID * peerId in self.session.connectedPeers) {
            NSString * displayNameOfPeerId = peerId.displayName;
            if ([peerIdDisplayname isEqualToString:displayNameOfPeerId]) {
                peerIds=[NSArray arrayWithObject:peerId];
                break;
            }
        }
    }
    [self.session sendData:messageData toPeers:peerIds withMode:MCSessionSendDataReliable error:&error];
...
- (Transcript *)sendImage:(NSURL *)imageUrl toPeerId:(NSString *)peerIdDisplayname
{
    NSProgress *progress;
    ...
    {
        for (MCPeerID * peerId in self.session.connectedPeers) {
            if (peerIdDisplayname ==peerId.displayName) {
                peerIds=[NSArray arrayWithObject:peerId];
                break;
            }
        }
